...
```bash
python basic_bench.py --result_img_folder <path_to_edited_images> --edit_json <path_to_edit_json> --origin_img_root <path_to_original_images> --num_processes <number_of_threads> --prompts_json <path_to_prompts_json>
```
...
```bash
python basic_bench.py --result_img_folder ./edited_images --edit_json ./edits.json --origin_img_root ./original_images --num_processes 4 --prompts_json ./prompts.json
```
## Example Input/Output
...
```bash
python step1_get_avgscore.py --result_json ./result.json --average_score_json ./average_scores.json
```
...
### Output
A JSON file (`average_scores.json`) with average scores for each image:
...
### Example
```bash
python step2_typescore.py --average_score_json ./average_scores.json --typescore_json ./type_scores.json --basic_edit ./basic_edit.json
```
...
### Output
A JSON file (`type_scores.json`) with average scores for each category:
...
You can run all three steps with a single shell command. First, organize your files as follows:
...
your_project_folder/
├── Basic/
│   ├── run_eval.sh                 # The evaluation script
│   ├── basic_bench.py
│   ├── step1_get_avgscore.py
│   ├── step2_typescore.py
│   ├── imgeasy_edit.json           # Edit metadata
│   ├── basic_edit.json             # Edit types
│   └── prompts.json                # Evaluation prompts
├── Benchmark/
│   ├── imgeasy_original_images/    # Original images directory
...
```bash
bash run_eval.sh <path_to_edited_images>
```

### Example

```bash
bash run_eval.sh ./Benchmark/edited_images
```

The script will automatically:
1. Run `basic_bench.py` to evaluate edited images using GPT
2. Run `step1_get_avgscore.py` to calculate average scores
3. Run `step2_typescore.py` to calculate scores by edit type

### Output Files

All results will be saved in the specified edited images folder:

```
<path_to_edited_images>/
├── result.json          # Raw GPT evaluation results
├── average_scores.json  # Average scores for each image
└── type_scores.json     # Scores grouped by edit type
```

### Notes

- Make sure you have a valid OpenAI API key configured in `basic_bench.py`
- The script uses 8 processes by default for parallel evaluation
