...
# ==========================================
# Get the ATR_Framework root directory (parent of core/)
...
# ==========================================
# 3. Per-Class System Prompts (loaded from txt files)
# ==========================================
# Load prompts from prompts_qwen directory (not from core)
SYSTEM_PROMPT_DIR = os.path.join(ATR_ROOT, "prompts_qwen")
...
def _load_class_prompt(filepath):
    ...
    try:
        with open(filepath, 'r', encoding='utf-8') as f:
            content = f.read()
        # Strip Python assignment wrapper if present
...
# ==========================================
# 4. Agent Session Class (The Brain & Hands)
# ==========================================
class AgentSession:
    def __init__(self, index, original_instruction, working_instruction, original_path, output_dir, editor_pipe, routing_class=None):
        self.index = str(index)
        self.original_instruction = original_instruction
        self.instruction = working_instruction
        self.original_path = original_path
        self.output_dir = output_dir
        self.editor_pipe = editor_pipe
        self.routing_class = routing_class  # A1 / A2 / B / C
        ...
        # Data Caches for sam3 and target tools
    ...
    def _draw_bboxes(self, img_path, orig_bbox, target_bbox, out_path):
        """Helper to create visualization for target_tool"""
        try:
    ...
    def _call_gemini_planner(self, prompt_text, image_path=None):
        ...
        # Select system prompt based on routing class
    ...
    def _parse_step(self, text):
        ...
        # 2. Extract Step
        step_match = re.search(r"<step>\s*(.*?)\s*</step>", text, flags=re.S)
        ...
        try:
            ...
            # Parse JSON directly using standard library
            step_data = json.loads(body)
            
        except json.JSONDecodeError as e:
            # Fallback logic: handle non-standard "key: value" output format
            lines = [line.strip() for line in body.split('\n') if ':' in line]
            if lines:
                for line in lines:
                    try:
                        k, v = line.split(':', 1)
                        # Clean up whitespace, quotes, and commas from keys and values
                        k = k.strip().strip('"\'').strip()
                        v = v.strip().strip('"\',').strip()
                        
                        # If value looks like a list, parse it
                        if v.startswith('[') and v.endswith(']'):
                            raw_items = [item.strip().strip('"\'') for item in v[1:-1].split(',')]
                            parsed_list = []
                        ...
                        continue  # Skip lines that cannot be split by colon
            
            # If fallback parsing fails to extract "tool", declare complete failure
            if not step_data or "tool" not in step_data:
                print(f"   [Parse Error] Invalid JSON in <step>. Error: {e}")
                print(f"   Raw string: {body}")
                return thought, None, False

        # For crop_keywords: always ensure it's a list
        if "crop_keywords" in step_data and isinstance(step_data["crop_keywords"], str):
            step_data["crop_keywords"] = [step_data["crop_keywords"]]
        elif "crop_keyword" in step_data:
            kw = step_data.pop("crop_keyword")
            step_data["crop_keywords"] = [kw] if isinstance(kw, str) else kw
    ...
    def _execute_tool(self, step_data, step_idx):
        ...
        try:
            if tool == "sam3_tool":
                ...
                # Support both crop_keywords (list) and crop_keyword (string) from model
                ...
                source_name = step_data.get("source") or step_data.get("source(base)")
                src = get_path(source_name)
                crop_step_name = step_data.get("crop_step")
                target_step_name = step_data.get("target_step")
                
                # Get cutout path from SAM3 cache
                cutout_path = self.step_sam3_cache.get(crop_step_name)
                
                if target_step_name and target_step_name in self.step_bbox_cache:
                    target_bbox = self.step_bbox_cache.get(target_step_name)
                else:
                    target_bbox = self.step_bbox_cache.get(crop_step_name)
                if cutout_path and target_bbox:
                    ...
                    shutil.copy(src, final_path)
                    step_data["ERROR"] = f"smartpaste_tool failed: cutout or original bbox not found for crop_step='{crop_step_name}'. Ensure sam3_tool completed successfully first."
                    
            elif tool == "crop_tool":
                src = get_path(step_data.get("source"))
                bbox = step_data.get("bbox_rel1000")
                if not bbox: raise ValueError("crop_tool missing bbox")
                ...
                with torch.inference_mode():
                    res = self.editor_pipe(image=[img], prompt=prompt, negative_prompt=" ", num_images_per_prompt=1).images[0]
                res.save(final_path)
                ...
                if not bbox: raise ValueError("croppaste missing bbox")
                ...
                img = Image.open(src).convert("RGB")
                with torch.inference_mode():
                    res = self.editor_pipe(image=[img], prompt=gen_prompt, negative_prompt=" ", num_images_per_prompt=1).images[0]
                res.save(final_path)
    ...
    def run_loop(self):
        ...
        while self.step_count < self.max_steps:
            ...
            if status == "success":
                ...
                # If ifinish_tool returned true, editing is done — skip planner entirely
...
def gpu_worker(rank, world_size, data_list):
    ...
    for item in my_chunk:
        ...
        routing_class = item.get("class", "A1")  # A1 / A2 / B / C from jsonl
        ...
        try:
            ...
            session = AgentSession(idx, instr, working_instr, img_path, sample_dir, editor_pipe,
...
def main():
    ...
    # Auto-restart logic with intelligent cleanup and max restart attempts
    MAX_RESTARTS = 5
    iteration = 1
    ...
    while True:
        ...
        for item in data:
            ...
            if os.path.exists(trace_path):
                ...
                # Trace file not found, mark as pending
                pending_ids.append(idx)
